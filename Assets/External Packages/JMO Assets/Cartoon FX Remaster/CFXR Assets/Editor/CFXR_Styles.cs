...
namespace CartoonFX
{
    public static class Styles
    {
        ...
        public static void HelpBoxRichText(Rect position, string message, MessageType msgType)
        {
            ...
            switch (msgType)
            {
                case MessageType.Warning:
                    icon = warnIcon ?? (warnIcon = EditorGUIUtility.Load("console.warnicon") as Texture2D);
                    break;
                case MessageType.Info:
                    icon = infoIcon ?? (infoIcon = EditorGUIUtility.Load("console.infoicon") as Texture2D);
                    break;
                case MessageType.Error:
                    icon = errorIcon ?? (errorIcon = EditorGUIUtility.Load("console.erroricon") as Texture2D);
                    break;
            }
        ...
        public static void MaterialDrawSeparator()
        {
            ...
            if (EditorGUIUtility.isProSkin)
            {
                DrawLine(new Color(.3f, .3f, .3f, 1f));
            }
            else
            {
                DrawLine(new Color(.6f, .6f, .6f, 1f));
            }
        ...
        public static void MaterialDrawSeparatorDouble()
        {
            ...
            if (EditorGUIUtility.isProSkin)
            {
                DrawLine(new Color(.1f, .1f, .1f, 1f));
                DrawLine(new Color(.4f, .4f, .4f, 1f));
            }
            else
            {
                DrawLine(new Color(.3f, .3f, .3f, 1f));
                DrawLine(new Color(.9f, .9f, .9f, 1f));
            }
